...
mkFeature {
  ...
  globals =
    { config, pkgs, ... }:
    {
      apps.launcher =
        with config.ordenada.features.rofi;
        lib.mkIf enableLauncher (
          lib.mkForce ''
            "${package}/bin/rofi -show drun"
          ''
        );
      ordenada.globals.passwordManager =
        let
          ## TODO: We assume we use `pass` to manage passwords.
          ...
          rofiPassExec =
            ...
            pkgs.writeShellScriptBin "rofi-pass" ''
              ...
              exec "${rofi.package}/bin/rofi-pass" "$@"
            '';
        in
        with config.ordenada.features.rofi;
