...
(defun ordenada-json5-format-buffer (&optional skip-cursor-restore)
...
With a prefix argument, skips cursor position restoration."
  (interactive "P") ; P means 'prefix argument', which will set skip-cursor-restore to t if present.
  ...
  (let* ((formatjson5-path "${pkgs.formatjson5}/bin/formatjson5")
         ...
         (original-node (unless skip-cursor-restore (treesit-node-at (point)))) ; Only capture if not skipping
        ...
        (progn
          ...
          (if (zerop exit-code)
              (let ((formatted (with-current-buffer output-buffer (buffer-string))))
                ...
                ;; Try to find a matching node to restore point ONLY if not skipping
...
(require 'cl-lib) ;; Ensure cl-lib is loaded for cl-defun
